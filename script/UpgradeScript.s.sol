...
contract UpgradeScript is Script {
    ...
    function run(address ADMIN_ADDRESS, address PROXY_ADDRESS) public {
        ...
        console.log("Upgrade successful! New logic at:", address(counterUpgrade));
    }

    function deployUpgradeContract(address ADMIN_ADDRESS, address PROXY_ADDRESS) public {
        counterUpgrade = new CounterUpgrade();
        ProxyAdmin admin = ProxyAdmin(ADMIN_ADDRESS);
        address owner = admin.owner();
        console.log("current owner : ", owner);
        vm.startBroadcast(owner);
        admin.upgradeAndCall(ITransparentUpgradeableProxy(PROXY_ADDRESS), address(counterUpgrade), new bytes(0));
        vm.stopBroadcast();
    }
}
